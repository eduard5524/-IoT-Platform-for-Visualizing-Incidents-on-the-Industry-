...
import { ConfirmationAcknowComponent } from './acknowledge-confirmation/acknowledge-confirmation.component';
...
export class AcknowComponent implements OnInit {
   idalarm: any;
   iduser: any;
   location: any;
   alarms: Alarm[];
  ...
  ngOnInit() {
    this.idalarm = this.dialogData.idalarm;
    this.location = this.dialogData.location;
    this.iduser = this.sessionService.getUserId();
  }

  deactivateThis(idalarm: any) {
    //llamada a servicio
    const mensaje = this.translate.instant('ACKNOW.alarm_deactivate_process');
  ...
  deactivateAll() {
    this.dialog.open(ConfirmationAcknowComponent, {
      width: '600px',
      data: {
